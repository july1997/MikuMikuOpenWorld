...
public:
		...
		, socket_(*io_service_)
	{
		...
		std::cout << "connect : " << "conection[" << my_conection << "]" << std::endl;
	}
	~Sever(){}
	...
	void receive_end(const boost::system::error_code& error)
	{
		if (error) {
			...
			if (length != std::string::npos)
			{
				...
				//暗号化初期化
				if (bit == std::bitset<7>(b_initcrypto))
				{
					if (!init)
					{
						getRsaPublicKey();
					...
					{
						std::cout << "Crypto : " << "has already initialized" << " conection[" << my_conection << "]" << std::endl;
					}
				}
...
public:
	...
	void conect()
	{
		while (1)
		{
			...
			//受信開始
			server[connection]->receive_start();
...
int main()
{
	std::cout << "Boost asio sever" << std::endl;
